...
WORKDIR /app/service
COPY leasebase-bff-gateway/package.json leasebase-bff-gateway/package-lock.json* ./
RUN npm ci
COPY leasebase-bff-gateway/tsconfig.json ./
COPY leasebase-bff-gateway/src ./src
RUN npm run build
